...
The opt-in `inject_px4_z_failure:=true` launch node refuses to run unless `/use_sim_time=true`, `ROS_MASTER_URI` uses port 11319, and MAVROS FCU URL is exactly `udp://:14540@localhost:14557`; its 5 guard/cleanup unit tests pass. It sets `SYS_FAILURE_EN=1` only on the isolated SITL, triggers failure at the configured depth, and now immediately returns that permission to 0 after both failure commands are accepted. A separate short SITL recheck saw `INJECTED_BARO_GPS_OFF_PERMISSION_DISABLED` and read back `SYS_FAILURE_EN=0` before shutdown. If a run aborts before that status, the parameter may remain 1 across SITL restarts: check and reset it explicitly before another run. An earlier aborted run did leave it at 1; it was subsequently reset to 0 and read back. The real FCU was never modified.
