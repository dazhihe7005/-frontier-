...
**2026-09-21 update:** the latest Task 2 code also passed an isolated **500 m** PX4/Gazebo nominal cycle using a 0.3–8 m downward rangefinder plus a SITL-only side-optical-flow integrated depth surrogate. The task layer does not require PX4 absolute Z or PX4/depth agreement. Minimum bottom range was 3.646 m, command acceleration stayed at or below 0.5 m/s², and the run completed into AUTO.LOITER. This remains a simulation interface/failure-model result, not a real-flight safety case. See [500 m optical-flow acceptance](task2_500m_opticalflow_acceptance_2026-09-21.md).
...
The algorithm records entrance depth on activation. The current downward-laser contract starts acceleration-limited braking at a fresh reading of **5.0 m or less**, requires 0.3 s consecutive bottom detections, and then uses the same acceleration limit while reversing into the return leg. Stale input, a 2 m depth jump, the 450 m depth limit, or 1800 s time limit enters FAULT. Invalid parameters fail before motion. The real entrance-relative depth input is intended to come from side-looking optical-flow integration, but that producer and its validation remain deferred until the hardware is installed; the downward laser is only the bottom trigger and is not fused into EKF. This is a state-machine contract, not a flight safety case.

## Evidence

`catkin_make -C /home/nuc/super_ws shaft_mission_node test_shaft_mission` compiled. Nine GTests pass, including the exact 5.00 m braking boundary, acceleration-limited reversal, and an **idealized** 420 m kinematic loop that descended until the bottom became visible, triggered return and reached the starting depth. A ROS-topic integration test also passes: enable→descent→confirmed bottom→return→complete, then stale-range reset→fault. The first integration attempt correctly faulted because the test injected an impossible 0→5 m depth jump; after correcting that test input and rebuilding the node, the test passed. The model assumes perfect independent depth and bottom range measurements; it does not represent PX4 dynamics, barometer drift, sensor dropouts over hundreds of metres, walls, airflow, communications loss or battery reserves. The 22 m PX4/Gazebo probe below extends interface coverage but still does not address these real-world conditions.
