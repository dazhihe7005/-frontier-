...
为降低实测网格、点云离散化和管线对任务级连通图的偶发影响，又完成了
以下安全约束：

- 仅白象山碰撞仿真启用“已实际穿越机体包络”自由空间修正；通用配置和
  真机默认关闭，避免定位误差擦除真实墙体；
- 首次死路仍必须有实时前墙证据；反转后允许以已稳定收敛、无可达前沿的
  机体可通行连通分量作为第二支路结束条件，不能用 Gazebo 真值补图；
- 普通前向目标在白象山只沿任务中心线搜索，并按“同高度优先、再缩短
  距离、最后才升降”排序；确认障碍后的横向绕行仍由独立净空检查负责；
- 从死路面包屑回到任务原点的最后航点使用 0.25 m 到达半径，不再用普通
  航点的 1 m 半径提前结束；反转后若入口只连通一个体素，可先走 0.5 m
  观测步获取新视角；
- 前向已知自由通道不再因为侧墙暂时不可见、进入前沿备用阶段而被阻断。
...
- `test/test_super_goal_publication.cpp`：9 项通过；
...
- 白象山场景中已动态确认 PX4 `connected/armed/OFFBOARD`、外部视觉
  `STREAMING`、自主前飞、死路航向约 180° 反转、逐个面包屑返回任务
  原点，并在一次连续运行中继续相反方向探索；
- 最新中心线/最终航点修正后的运行保持 FAST-LIO2 `STREAMING` 和 PX4
  `OFFBOARD`，沿中心线到达约 -16.2 m 后在局部车辆可达图不完整时安全
  等待，没有把未知区域伪装成自由空间或伪报任务完成；
- 尚未在同一次最新版本运行中取得“双向探索 → `MODEL_COMPLETE` → 沿
  面包屑回到原点”的完整终态证据。实测地图管线附近仍存在 SUPER 安全
  走廊接受率和点云连通性随运行变化的问题。
...
流、解锁、OFFBOARD、死路反转、面包屑回原点和反向继续的动态证据；
Task2 500 m 竖井以及白象山最新版本的完整终态
仍需分别完成最终验收：

- 记录 `/Odometry` 的长时间频率、漂移和协方差；
- 使用 FAST-LIO2 而非真值定位完成竖井起飞和至少 10 秒稳定下降；
- 在白象山地图一次连续运行中完成双向探索、`MODEL_COMPLETE` 和最终返航；
...
6. 已在白象山动态观察外部视觉闭环、自主前飞、死路反转和逐段回退；
7. 已在一次连续运行中观察面包屑回到任务原点并继续反向探索；
8. 继续收敛实测地图管线附近的点云连通性和 SUPER 安全走廊接受率，并
   取得最新版本 `MODEL_COMPLETE` 后最终返航的单次连续证据；
9. 重启 Task2，验证 PointCloud2 → FAST-LIO2 → PX4 外部视觉下降闭环；
10. 完成遗留 `iris`/`iris_vision` 入口分类审计；
11. 补充长时间稳定性、定位漂移和碰撞净空统计。
