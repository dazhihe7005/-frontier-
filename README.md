...
使用 `coverage_audit:=true` 时，启动脚本从当前碰撞 OBJ 重建二维和三维 0.5 m 离线可达参考；审计节点只读真值与原始射线，同时报告两种参考的可见率。三维参考不会进入定位、规划、控制或安全制动链路。完整验收要求两种参考、网格净空、返航与全链路报告一致；详见 [验收说明](src/mine_uav_gbplanner/docs/full_map_acceptance.md)。
...
- MID360S 的安装视场无法覆盖机体正下方的近地盲锥。隔离仿真中已将模型
  自带的向下测距移到起落架下方；后续针对斜坡漏检，将单束改成水平/垂直
  各 11 束、各 ±45° 的扇形，并估算机体中心到下方障碍的制动量；
...
且侧支斜地面实际网格净空最小仅 1.07256 m。新增的仅仿真下视扇形让
同组试验到达约 112 m、覆盖 44.87%，但在管线处最小净空又只有
