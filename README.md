...
- 实时防撞使用固定 1 m 安全半径，不设置硬性 `+X` 方向过滤。规划层水平
...
- 起飞交接要求位置、姿态和速度连续稳定 2 秒。外部视觉速度门槛为
  3.5 m/s，与 PX4 最高 3.0 m/s 起飞包络匹配。当前正常规划速度为经过
  长时验收的 0.5 m/s；2.0 m/s 试验因换段误差导致碰撞，已禁用。
...
- MID360 增加独立预测防撞：依据当前运动方向计算进入 1 m 包络前的剩余
  距离，普通航段从 1.40 m 制动余量开始降速、到 0.65 m 余量时停止；接近
  frontier 末端时使用 1.70/0.90 m 的更保守阈值。三维绝对净空从 1.55 m
  开始降速，到 1.25 m 停止；顶板/地面接近时另执行 0.35 m 垂直脱离。
...
- 局部树和全局 frontier 都持续返回空结果时，恢复节点会在 5 次空决策后
  ...
  按预估时长假定成功。回退仍保留 1 m 实测净空硬停止，不使用 Gazebo 真值。
...
最新验证记录见 [VALIDATION_2026-09-25.md](VALIDATION_2026-09-25.md)，早期
