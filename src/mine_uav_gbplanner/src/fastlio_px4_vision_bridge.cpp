 ...
 public:
  FastlioPx4VisionBridge(const ros::NodeHandle& nh,
    ...
    alignment_publisher_ = nh_.advertise<geometry_msgs::TransformStamped>(
        "/mine_uav/gbplanner/fastlio_to_px4_alignment", 1, true);
    healthy_publisher_ = nh_.advertise<std_msgs::Bool>(
        "/mine_uav/gbplanner/vision_healthy", 1, true);
    status_publisher_ = nh_.advertise<std_msgs::String>(
        "/mine_uav/gbplanner/vision_status", 1, true);
 ...
 private:
  ...
  void odometryCallback(const nav_msgs::Odometry::ConstPtr& odometry) {
    ...
    if (have_previous_input_) {
      ...
      double dt = (odometry->header.stamp - previous_input_stamp_).toSec();
      if (dt <= 0.0) {
        dt = (ros::Time::now() - last_input_time_).toSec();
      }
      const bool absolute_jump = distance > max_input_jump_;
  ...
  void timerCallback(const ros::TimerEvent&) {
    ...
    // FAST-LIO publishes at the lidar rate while this health timer runs at a
    // higher rate.  Re-sending one estimate with a fresh timestamp makes PX4
    // fuse the same delayed observation several times and hides the actual
    // lidar/estimator latency.  Publish every input exactly once; the source
    // stamp identifies the new frame while EKF2_EV_DELAY represents its
    // measured processing latency.
    const ros::Time measurement_stamp = latest_odometry_.header.stamp;
    if (measurement_stamp.isZero()) {
      publishHealth(false);
      publishStatus("INVALID_ODOMETRY_TIMESTAMP");
      return;
    }
    if (measurement_stamp != last_published_input_stamp_) {
      // MAVROS and PX4 SITL use the current synchronized transport time here;
      // the measured FAST-LIO processing latency is represented explicitly by
      // EKF2_EV_DELAY in this workspace's private 1018 airframe.  Feeding the
      // older ROS /clock stamp directly makes PX4 discard otherwise valid EV
      // samples before the EKF delay buffer is sized for them.
      pose_publisher_.publish(makeVisionPose(now));
      last_published_input_stamp_ = measurement_stamp;
    }
  ...
  ros::Time previous_input_stamp_;
  ros::Time last_published_input_stamp_;
