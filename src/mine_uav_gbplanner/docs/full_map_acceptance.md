...
后续隔离 A/B 仅为诊断仿真起点盲区，增加默认关闭的
`surveyed_launch_backtrack_m`。它在离线碰撞网格已核验的短走廊里注入
弱自由先验，不是飞行中由 MID360 测得的自由空间。预检脚本
`tools/audit_surveyed_launch_corridor.py` 对起点至后退 3 m 的中心线每
0.05 m 采样、扣除半步长及 0.10 m 位置误差后，给出至少 1.24254 m
的仿真网格净空下界；`real_site_survey_proven=false`。未经独立实地测绘，
不允许将此试验选项用于真机未知环境探索。

半径 1.0 m 的短试验在约 213 仿真秒里前进到 x≈-35 m，二维可见
986/3089、三维可见 2720/8460；采样最小真实网格净空 1.17219 m，
但未完成全图，且旧恢复模块一度把尚未飞过的规划命令倒放成回退路径。
恢复现已改为只倒放连续观测到的里程计位姿，跳变和定位失效立即丢弃
历史。新短试验约 193 仿真秒在 x≈-21 m 停滞，二维可见 754/3089、
三维可见 2117/8460；采样最小净空只有 1.06213 m（地面），虽未小于
1 m，却不满足额外 0.15 m 验收缓冲。扩大规划球半径到 1.15 m
的 75 秒试验更早陷入空图，不能简单靠增大半径替代原因排查。

在 x≈-20.84 m 的最小净空点，离线碰撞网格测得地面最近 1.062 m，
顶板最近 1.984 m，设施最近 1.248 m，岩壁最近 1.382 m；因此不能
把该处直接判为上下只有 2 m 的不可飞通道。实时 MID360 反而把向上
可动余量压到约 0.005–0.044 m，同时 PX4 控制下机体高度在近似原地
约 4 秒下降 0.28 m。已增加只读同步记录，待复现时对照 PX4 估计高度、
下发高度、地面测距和 MID360 向上余量，分清误回波、姿态/坐标变换、
高度估计漂移与执行误差；在因果链确认之前不得放松实时防撞阈值。

2026-09-27 同步记录把根因进一步缩小：一次约 405 仿真秒的旧高度控制
架次到达 x≈-64 m，但二维仅可见 1259/3089、三维仅可见 3482/8460，
真实网格最小净空 1.07524 m（设施），仍不满足 1.15 m 预检门槛。
在起飞期 23–30 秒，PX4 本地高度从约 1.23 上升到 1.31 m，Gazebo
真值从约 2.09 上升到 2.32 m，然而 PX4 报告的垂直速度却约为
-0.14 至 -0.23 m/s，方向与两种位置变化相反。PX4 ULog 的
`vehicle_local_position.z` 和 `vz` 也出现相同矛盾；不能把问题简单归为
MAVROS 坐标显示。

尝试给锁存的避顶板目标加 1.70 m 地面跟随下界后，另一架次虽通过
起点到达 x≈17 m，却在起飞期离顶板仅 1.00827 m，只有 8 mm
采样裕量，因此此控制改动已撤回。仅在隔离 PX4 SITL 将
`EKF2_EV_CTRL` 从 1 改为 3、保留气压计高度参考的 A/B 中，高度/速度
读数较一致，但严格球形规划器持续返回前方未知体素和短轨迹，约 70 秒
未离开起点；视觉 Z 融合改动亦已撤回。上述试验都不能代表全图、安全
或快速探索通过。鉴于两条控制方案都未过 1.15 m 初步裕量门槛，
`allow_auto_arm` 默认改为 `false`；任何再次自动解锁必须显式传入
`allow_auto_arm:=true`，仅用于有人监控的隔离仿真 A/B。
